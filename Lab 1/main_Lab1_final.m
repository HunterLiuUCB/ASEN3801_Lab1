%Contributors: Hunter Liu, Jessa Wanninger, Andrew Yates, and Parker Himes
%Course number: ASEN 3801
%File name: main_Lab1_final.m
%Created: 1/13/26
clear
clc
close all
...
opt = odeset('RelTol',rel_tol,'AbsTol',rel_tol); %setting ode45 tolerance options
...
% Inputs:  t = horizontal time vector
%          s = horizontal variable vector for ode definition
%            = [w x y z]
% Outputs: diff_eqs = column vector of unsolved ODEs
%
% Methodology: Uses a state vector 's' to plug variables into the ODEs
% which are located in the column vector diff_eqs. Allowing for easier
...
%
% Outputs: value = arbitrary value for vertical position
%          isterminal = arbitrary value to terminate ODE function
%          direction = arbitrary value to represent direction of movement
%
% Methodology: 
...
%
% Outputs: const = structure with predefined coefficient of drag, diameter,
%                  area, mass, and gravity outputs
%          const.rho = calculated air density from inputs in kg/m^3
%
% Methodology: Uses altitude and temperature offset to find the density
% using the 1976 standard atmopshere database. This function also has a
% structure that stores several geometric constants for use later.
